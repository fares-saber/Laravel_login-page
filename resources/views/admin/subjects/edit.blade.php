            ...
            <form action="{{ route('admin.subjects.update', $subject) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('name') border-red-500 @enderror" 
                           id="name" name="name" type="text" placeholder="Enter subject name" value="{{ old('name', $subject->name) }}">
                    @error('name')
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('code') border-red-500 @enderror" 
                           id="code" name="code" type="text" placeholder="Enter subject code" value="{{ old('code', $subject->code) }}">
                    @error('code')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
                </div>
